...
{
    const string BROKER_NAME = "zawodowcy_event_bus";
    const string AUTOFAC_SCOPE_NAME = "zawodowcy_event_bus";
    ...
    public void Subscribe<T, TH>()
        ...
        where TH : IEventBusEventHandler<T>
    {
        var eventName = _subsManager.GetEventKey<T>();
        DoInternalSubscription(eventName);

        _logger.LogInformation($"Subscribing to event {eventName} with {typeof(TH).ToString()}");
    ...
    private async Task ProcessEvent(string eventName, string message)
    {
        ...
        if (_subsManager.HasSubscriptionsForEvent(eventName))
        {
            ...
            foreach (var subscription in subscriptions)
            {
                ...
                var concreteType = typeof(IEventBusEventHandler<>).MakeGenericType(eventType);
